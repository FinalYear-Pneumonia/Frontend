  ...
  useEffect(() => {
    if (!token) {
      ...
      .then(response => {
        ...
        localStorage.setItem('emailForPasswordUpdate', response.data.email);
      })
      .catch(error => {
    ...
    <section className={styles.forg_pwd_section}>
      ...
      <form className={styles.forg_pwd_form} onSubmit={handleFormSubmit}>
        ...
        <input
          ...
          value={contact}
        />
        <button type="submit" className={styles.forg_pwd_btn} disabled={isLoading}>
      ...
      <p>
        <br />
        <span className="line">
          <a href="/updatepwd">Update Password?</a>
        </span>
        <br /><br />
        <span className="line">
